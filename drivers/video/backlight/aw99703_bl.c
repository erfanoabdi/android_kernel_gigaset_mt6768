...
//prize baibo for aw99703 i2c control begin
#if 0 
	if (drvdata->brightness == 0)
		drvdata->enable = false;
#endif
//prize baibo for aw99703 i2c control end
	return 0;
}
...
int chargepump_set_backlight_level(unsigned int level)
{
	...
	if (level > 0) {
		/* enalbe bl mode */
		/* set backlight level */
		//prize add by wangfei for sleep open backlight when sys resume 20210819 start
		if(g_aw99703_data->brightness == 0) 
		{
			mdelay(30);
		}
		//prize add by wangfei for sleep open backlight when sys resume 20210819 end
...
//prize baibo for aw99703 i2c control begin
EXPORT_SYMBOL_GPL(chargepump_set_backlight_level);
//prize baibo for aw99703 i2c control end
			...
			const struct i2c_device_id *id)
{
	...
	drvdata->enable = true;     //prize modify by wangfei for don't init backlight in probe 20210727
