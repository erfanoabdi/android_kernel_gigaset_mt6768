...
/*****************************************************************************
*
* File Name: focaltech_upgrade_ft8009.c
*
...
*****************************************************************************/
u8 pb_file_ft8009[] = {
#include "../include/pramboot/FT8009_Pramboot_v1.1_20190318.i"
};
...
static int fts_ft8009_get_hlic_ver(u8 *initcode)
{
...
static int fts_ft8009_upgrade_mode(enum FW_FLASH_MODE mode, u8 *buf, u32 len)
{
    int ret = 0;
    u32 start_addr = 0;
    u8 cmd[4] = { 0 };
    ...
    if (upgrade_func_ft8009.appoff_handle_in_ic) {
        start_addr = 0; /* offset handle in pramboot */
    } else {
        start_addr = upgrade_func_ft8009.appoff;
    }
...
/************************************************************************
* Name: fts_ft8009_upgrade
* Brief:
* Input:
* Output:
* Return: return 0 if success, otherwise return error code
***********************************************************************/
static int fts_ft8009_upgrade(u8 *buf, u32 len)
{
    int ret = 0;
    u8 *tmpbuf = NULL;
    u32 app_len = 0;
    ...
    app_len = len - upgrade_func_ft8009.appoff;
    tmpbuf = buf + upgrade_func_ft8009.appoff;
    ret = fts_ft8009_upgrade_mode(FLASH_MODE_APP, tmpbuf, app_len);
    if (ret < 0) {
...
static int fts_ft8009_lic_upgrade(u8 *buf, u32 len)
{
    int ret = 0;
    u8 *tmpbuf = NULL;
    u32 lic_len = 0;
    ...
    ret = fts_ft8009_upgrade_mode(FLASH_MODE_LIC, tmpbuf, lic_len);
    if (ret < 0) {
...
struct upgrade_func upgrade_func_ft8009 = {
    .ctype = {0x17},
    .fwveroff = 0x110E,
    .fwcfgoff = 0xF80,
    .appoff = 0x1000,
    ...
    .pramboot = pb_file_ft8009,
    .pb_length = sizeof(pb_file_ft8009),
    .pram_ecc_check_mode = ECC_CHECK_MODE_CRC16,
    .new_return_value_from_ic = true,
    .hid_supported = false,
    .upgrade = fts_ft8009_upgrade,
    .get_hlic_ver = fts_ft8009_get_hlic_ver,
    .lic_upgrade = fts_ft8009_lic_upgrade,
};
