...
u32 cts_crc32(const u8 *data, size_t len)
{
    ...
    while (len) {
        lcrc = (lcrc << 8) ^ crc32_table[((lcrc >> 24) ^ *data) & 0xFF];
...
static int get_lcd_idx_index(void)
{
     char *ptr;
     int lcd_idx_index=0;
     ptr=strstr(saved_command_line,"lcd_id=");
     if(ptr==NULL)
	     return -1;
     ptr+=strlen("lcd_id=");
     lcd_idx_index = simple_strtol(ptr,NULL,10);
     return lcd_idx_index;
} 
    ...
    for (i = 0; i < ARRAY_SIZE(cts_driver_builtin_firmwares); i++, firmware++) {
        if (MATCH_HWID(firmware, hwid) && MATCH_FWID(firmware, fwid)) {
            ...
            cts_info("Found matched driver builtin '%s' "
                ...
                firmware->name, firmware->hwid, firmware->fwid,
                firmware->size, FIRMWARE_VERSION(firmware));
			if(get_lcd_idx_index != 0 && fwid != CTS_DEV_FWID_ICNL9911S && hwid != CTS_DEV_HWID_ICNL9911S)
			{
				if(1 == get_lcd_idx_index() )
					return &cts_driver_builtin_firmwares[1];
				if(2 == get_lcd_idx_index())
					return &cts_driver_builtin_firmwares[2];
				if(3 == get_lcd_idx_index())
					return &cts_driver_builtin_firmwares[3];
				printk("prize cts tp lcd_id =%d \n",get_lcd_idx_index());
			}
