...
/*#if defined(CONFIG_PRIZE_HARDWARE_INFO)
	...
	ic_data->vendor_sensor_id = 0x83112;
	
	sprintf(current_tp_info.chip,"hx83112, fw_version:D%2.2XC%2.2X", ic_data->vendor_display_cfg_ver, ic_data->vendor_touch_cfg_ver);
	sprintf(current_tp_info.id,"0x%04x",ic_data->vendor_sensor_id);
	strcpy(current_tp_info.vendor,"Himax");
	sprintf(current_tp_info.more,"%d*%d",ic_data->HX_X_RES,ic_data->HX_Y_RES);
#endif	
*/
//prize add by lipengpeng 20220810 end 
...
void himax_mcu_pin_reset_sleep(void)
{
	I("%s: Now reset the Touch chip.\n", __func__);
	himax_rst_gpio_set(private_ts->rst_gpio, 0);
	usleep_range(RST_LOW_PERIOD_S, RST_LOW_PERIOD_E);
