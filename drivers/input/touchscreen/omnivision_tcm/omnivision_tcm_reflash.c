...
static enum update_area reflash_compare_id_info(void)
{
	...
	printk("wolf image_fw_id = %d,device_fw_id = %d\n",le4_to_uint(header->build_id),tcm_hcd->packrat_number);
...
//prize add by wangfei for close compare fw id update 20210716 start
#if 0   
...
static int reflash_do_reflash(void)
{
	...
	case FIRMWARE_CONFIG:
		...
		if (tcm_hcd->features.dual_firmware) {
			...
			LOGN(tcm_hcd->pdev->dev.parent,
			"-------reflash_update_prod_test_firmware\n");
			if (retval < 0) {
				LOGE(tcm_hcd->pdev->dev.parent,
	...
	LOGN(tcm_hcd->pdev->dev.parent,
			"End of reflash\n");
	//prize add by wangfei for TP reset when TP fw update finish 20210715 start
	...
	// 	goto exit;
	// }
	//prize add by wangfei for TP reset when TP fw update finish 20210715 end
	retval = 0;
