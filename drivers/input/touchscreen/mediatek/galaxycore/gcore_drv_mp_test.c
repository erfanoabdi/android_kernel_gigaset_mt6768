...
#define MP_OPEN_TEST_DATA_FILEPATH    MP_TEST_DATA_DIR"/OpenTest"
#define MP_SHORT_TEST_DATA_FILEPATH   MP_TEST_DATA_DIR"/ShortTest"
#define MP_RAWDATA_TEST_DATA_FILEPATH   MP_TEST_DATA_DIR"/Rawdata"
#define MP_RAWDATA_TEST_HDEC_FILEPATH   MP_TEST_DATA_DIR"/Rawdata_Dec_H"
#define MP_RAWDATA_TEST_LDEC_FILEPATH   MP_TEST_DATA_DIR"/Rawdata_Dec_L"
#define MP_RAWDATA_TEST_HAVG_FILEPATH   MP_TEST_DATA_DIR"/Rawdata_Avg_H"
#define MP_RAWDATA_TEST_LAVG_FILEPATH   MP_TEST_DATA_DIR"/Rawdata_Avg_L"
#define MP_NOISE_TEST_DATA_FILEPATH     MP_TEST_DATA_DIR"/NoiseTest"
			...
			const u16 *data, int rows, int cols, enum MP_DATA_TYPE types)
{
	...
	switch (types) {
	case RAW_DATA:
		len = scnprintf(linebuf, sizeof(linebuf), "Max:, %4u, ,Min:, %4u, \n", 
...
int gcore_save_mp_data_to_file(struct gcore_mp_data *mp_data)
{
	...
	if (mp_data->test_open) {
		scnprintf(filepath, FILEPATH_MAX_LENGTH, "%s_%lu_%s.csv", 
					MP_OPEN_TEST_DATA_FILEPATH, tv.tv_sec, 
					mp_data->open_test_result ? "FAIL" : "PASS");
	...
	if (mp_data->test_short) {
		scnprintf(filepath, FILEPATH_MAX_LENGTH, "%s_%lu_%s.csv", 
					MP_SHORT_TEST_DATA_FILEPATH, tv.tv_sec, 
					mp_data->short_test_result ? "FAIL" : "PASS");
		ret = dump_mp_data_to_csv_file(filepath,
	...
	if (mp_data->test_rawdata) {
		scnprintf(filepath, FILEPATH_MAX_LENGTH, "%s_%lu_%s.csv", 
					MP_RAWDATA_TEST_DATA_FILEPATH, tv.tv_sec, 
					mp_data->rawdata_test_result ? "FAIL" : "PASS");
		ret = dump_mp_data_to_csv_file(filepath,
					     O_RDWR | O_CREAT | O_TRUNC, mp_data->rawdata_node_val, rows,
					     cols, RAW_DATA);
		if (ret < 0) {
...
#if MP_RAWDATA_DEC_TEST
		scnprintf(filepath, FILEPATH_MAX_LENGTH, "%s_%lu_%s.csv", 
					MP_RAWDATA_TEST_HDEC_FILEPATH, tv.tv_sec, 
					mp_data->all_test_result ? "FAIL" : "PASS");
		ret = dump_mp_data_to_csv_file(filepath,
					     O_RDWR | O_CREAT | O_TRUNC, mp_data->rawdata_dec_h, rows,
					     cols, RAWDATA_DECH);
		if (ret < 0) {
		...
		scnprintf(filepath, FILEPATH_MAX_LENGTH, "%s_%lu_%s.csv", 
					MP_RAWDATA_TEST_LDEC_FILEPATH, tv.tv_sec, 
					mp_data->all_test_result ? "FAIL" : "PASS");
		ret = dump_mp_data_to_csv_file(filepath,
...
#if MP_RAWDATA_AVG_TEST
		scnprintf(filepath, FILEPATH_MAX_LENGTH, "%s_%lu_%s.csv", 
					MP_RAWDATA_TEST_HAVG_FILEPATH, tv.tv_sec, 
					mp_data->all_test_result ? "FAIL" : "PASS");
		ret = dump_mp_data_to_csv_file(filepath,
		...
		scnprintf(filepath, FILEPATH_MAX_LENGTH, "%s_%lu_%s.csv", 
					MP_RAWDATA_TEST_LAVG_FILEPATH, tv.tv_sec, 
					mp_data->all_test_result ? "FAIL" : "PASS");
		ret = dump_mp_data_to_csv_file(filepath,
	...
	if (mp_data->test_noise) {
		scnprintf(filepath, FILEPATH_MAX_LENGTH, "%s_%lu_%s.csv", 
					MP_NOISE_TEST_DATA_FILEPATH, tv.tv_sec, 
					mp_data->noise_test_result ? "FAIL" : "PASS");
		ret = dump_mp_data_to_csv_file(filepath,
						   O_RDWR | O_CREAT | O_TRUNC, mp_data->noise_node_val, rows,
						   cols, NOISE);
		if (ret < 0) {
