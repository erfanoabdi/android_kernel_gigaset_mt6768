...
#if GCORE_WDT_RECOVERY_ENABLE
static u8 wdt_valid_values[] = { 0xFF,0xFF,0x73,0x02,0x00,0x01,0xFF,0xFF };
#endif
...
int gcore_probe_device(void)
{
	...
	while (retries++ < 3) {
		...
		if ((id == chip_type) || (id == chip_type_2)) {
   ...
   #ifdef CONFIG_PRIZE_HARDWARE_INFO
	strcpy(current_tp_info.chip, "gcore,fw_version:0x03");
	sprintf(current_tp_info.id, "0x%x",id);
	strcpy(current_tp_info.vendor, "Galaxycore");
	sprintf(current_tp_info.more, "%d*%d", 1080, 2412);
    #endif
	return ret;
}
