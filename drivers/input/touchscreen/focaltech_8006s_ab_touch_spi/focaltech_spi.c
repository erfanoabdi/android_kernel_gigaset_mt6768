...
*****************************************************************************/
/* spi interface */
static int fts_8006ab_spi_transfer(u8 *tx_buf, u8 *rx_buf, u32 len)
{
    int ret = 0;
    struct spi_device *spi = fts_8006ab_data->spi;
...
int fts_8006ab_write(u8 *writebuf, u32 writelen)
{
    ...
    for (i = 0; i < SPI_RETRY_NUMBER; i++) {
        ret = fts_8006ab_spi_transfer(txbuf, rxbuf, txlen);
...
int fts_8006ab_write_reg(u8 addr, u8 value)
{
    ...
    writebuf[0] = addr;
    writebuf[1] = value;
    return fts_8006ab_write(writebuf, 2);
}
...
{
    return fts_8006ab_read(&addr, 1, value, 1);
}
