...
struct upgrade_module module_list1[] = {
    {FTS_MODULE_ID, FTS_MODULE_NAME, fw8006_file, sizeof(fw8006_file)},
    {FTS_MODULE2_ID, FTS_MODULE2_NAME, fw8006_file2, sizeof(fw8006_file2)},
    {FTS_MODULE3_ID, FTS_MODULE3_NAME, fw8006_file3, sizeof(fw8006_file3)},
};

struct upgrade_setting_nf upgrade_8006ab__setting_list[] = {
    {0x86, 0x32, 0, (64 * 1024), (128 * 1024), 0xA5, 0x01, 12, 0, 0, 0, 0},
};

struct fts_8006ab_upgrade *fwupgrade8006ab;

static int fts_8006ab_check_bootid(void)
{
    int ret = 0;
    u8 cmd = 0;
    u8 id[2] = { 0 };
    struct fts_8006ab_upgrade *upg = fwupgrade8006ab;
    ...
    cmd = FTS_CMD_READ_ID;
    ret = fts_8006ab_read(&cmd, 1, id, 2);
    if (ret < 0) {
...
static int fts_8006ab_fwupg_hardware_reset_to_boot(void)
{
    fts_8006ab_reset_proc(0);
    return 0;
}

static int fts_8006ab_enter_into_boot(void)
{
    int ret = 0;
    int i = 0;
    int j = 0;
...
static bool fts_8006ab_check_fast_download(void)
{
    int ret = 0;
    ...
    ret = fts_8006ab_read_reg(0xdb, &value);
    if (ret < 0) {
        FTS_ERROR("read 0xdb fail");
        goto read_err;
    }

    ret = fts_8006ab_read(cmd, 6, value2, 2);
    if (ret < 0) {
...
static int fts_8006ab_dpram_write_pe(u32 saddr, const u8 *buf, u32 len, bool wpram)
{
    int ret = 0;
    int i = 0;
    int j = 0;
    ...
    if (upg->setting_nf->fd_check) {
        fd_support = fts_8006ab_check_fast_download();
        if (!fd_support)
    ...
    for (i = 0; i < packet_number; i++) {
        ...
        ret = fts_8006ab_write(&cmd[0], FTS_CMD_WRITE_LEN + packet_len);
        if (ret < 0) {
...
static int fts_8006ab_dpram_write(u32 saddr, const u8 *buf, u32 len, bool wpram)
{
    int ret = 0;
    int i = 0;
    int j = 0;
    ...
    for (i = 0; i < packet_number; i++) {
        ...
        cmd[2] = BYTE_OFF_8(addr);
        cmd[3] = BYTE_OFF_0(addr);
        ret = fts_8006ab_write(&cmd[0], FTS_ROMBOOT_CMD_SET_PRAM_ADDR_LEN);
        if (ret < 0) {
        ...
        ret = fts_8006ab_write(&cmd[0], 1 + packet_len);
        if (ret < 0) {
            FTS_ERROR("write fw to pram(%d) fail", i);
            goto write_pram_err;
        }
...
static int fts_8006ab_ecc_cal_tp(u32 ecc_saddr, u32 ecc_len, u16 *ecc_value)
{
    int ret = 0;
    int i = 0;
    ...
    ret = fts_8006ab_write(cmd, FTS_ROMBOOT_CMD_ECC_NEW_LEN);
    if (ret < 0) {
    ...
    for (i = 0; i < FTS_ECC_FINISH_TIMEOUT; i++) {
        ret = fts_8006ab_read(cmd, 1, value, 1);
        if (ret < 0) {
    ...
    ret = fts_8006ab_read(cmd, 1, value, 2);
    if (ret < 0) {
...
static int fts_8006ab_ecc_cal_host(const u8 *data, u32 data_len, u16 *ecc_value)
{
    u16 ecc = 0;
    u32 i = 0;
    u32 j = 0;
...
static int fts_8006ab_ecc_check(const u8 *buf, u32 len, u32 ecc_saddr)
{
    int ret = 0;
    int i = 0;
    u16 ecc_in_host = 0;
    ...
    for (i = 0; i < packet_number; i++) {
        ...
        ret = fts_8006ab_ecc_cal_host(buf + offset, packet_length, &ecc_in_host);
        if (ret < 0) {
            FTS_ERROR("ecc in host calc fail");
            return ret;
        }

        ret = fts_8006ab_ecc_cal_tp(ecc_saddr + offset, packet_length, &ecc_in_tp);
        if (ret < 0) {
...
static int fts_8006ab_pram_write_ecc(const u8 *buf, u32 len)
{
    int ret = 0;
    u32 pram_app_size = 0;
    u16 code_len = 0;
    ...
    if (upg->setting_nf->spi_pe)
        ret = fts_8006ab_dpram_write_pe(pram_start_addr, buf, pram_app_size, true);
    else
        ret = fts_8006ab_dpram_write(pram_start_addr, buf, pram_app_size, true);
    if (ret < 0) {
    ...
    /* check ecc */
    ret = fts_8006ab_ecc_check(buf, pram_app_size, pram_start_addr);
    if (ret < 0) {
...
static int fts_8006ab_dram_write_ecc(const u8 *buf, u32 len)
{
    int ret = 0;
    u32 dram_size = 0;
    u32 pram_app_size = 0;
    ...
    /* write pram */
    ret = fts_8006ab_dpram_write(dram_start_addr, dram_buf, dram_size, false);
    if (ret < 0) {
    ...
    /* check ecc */
    ret = fts_8006ab_ecc_check(dram_buf, dram_size, dram_start_addr);
    if (ret < 0) {
...
static int fts_8006ab_pram_start(void)
{
    int ret = 0;
 ...
 */
static int fts_8006ab_fw_write_start(const u8 *buf, u32 len, bool need_reset)
{
    int ret = 0;
    struct fts_8006ab_upgrade *upg = fwupgrade8006ab;
    ...
    if (need_reset) {
        ...
        ret = fts_8006ab_enter_into_boot();
        if (ret < 0) {
    ...
    /* write pram */
    ret = fts_8006ab_pram_write_ecc(buf, len);
    if (ret < 0) {
        FTS_ERROR("write pram fail");
        return ret;
    }
    ...
    if (upg->setting_nf->drwr_support) {
        /* write dram */
        ret = fts_8006ab_dram_write_ecc(buf, len);
        if (ret < 0) {
            FTS_ERROR("write dram fail");
            return ret;
        }
...
static int fts_8006ab_fw_download(const u8 *buf, u32 len, bool need_reset)
{
    int ret = 0;
    int i = 0;
    struct fts_8006ab_upgrade *upg = fwupgrade8006ab;
    ...
    for (i = 0; i < 3; i++) {
        ...
        ret = fts_8006ab_fw_write_start(buf, len, need_reset);
        if (0 == ret)
            break;
    }
    if (i >= 3) {
...
static int fts_8006ab_read_file(char *file_name, u8 **file_buf)
{
    int ret = 0;
...
int fts_8006ab_upgrade_bin(char *fw_name, bool force)
{
    ...
    ret = fts_8006ab_read_file(fw_name, &fw_file_buf);
    ...
    ret = fts_8006ab_fw_download(fw_file_buf, fw_file_len, true);
    if (ret < 0) {
...
int fts_8006ab_enter_test_environment(bool test_state)
{
    ...
    for (i = 0; i < 3; i++) {
        FTS_INFO("fw download times:%d", i + 1);
        ret = fts_8006ab_fw_write_start(upg->fw + app_offset, upg->fw_length, true);
        if (0 == ret)
            break;
    }
    upg->ts_data->fw_loading = 0;
    ...
    msleep(50);
    ret = fts_8006ab_read_reg(FTS_REG_FACTORY_MODE_DETACH_FLAG, &detach_flag);
...
int fts_8006ab_fw_resume(bool need_reset)
{
    ...
    ret = fts_8006ab_fw_download(fw_buf, fwlen, need_reset);
    if (ret < 0) {
...
int fts_8006ab_fw_recovery(void)
{
    ...
    ret = fts_8006ab_read_reg(0xD0, &boot_state);
    if (ret < 0) {
    ...
    ret = fts_8006ab_fw_resume(false);
    if (ret < 0) {
        FTS_ERROR("fts_8006ab_fw_resume fail");
        return ret;
    }
    msleep(10);
    ret = fts_8006ab_read_reg(FTS_REG_CHIP_ID, &chip_id);
    FTS_INFO("read chip id:0x%02x", chip_id);

    fts_8006ab_tp_state_recovery(upg->ts_data);

    FTS_INFO("boot recovery pass");
    return ret;
}

static int fts_8006ab_fwupg_get_module_info(struct fts_8006ab_upgrade *upg)
{
    int i = 0;
    struct upgrade_module *info = &module_list1[0];
...
static int fts_8006ab_get_fw_file_via_request_firmware(struct fts_8006ab_upgrade *upg)
{
    int ret = 0;
    const struct firmware *fw = NULL;
    u8 *tmpbuf = NULL;
...
static int fts_8006ab_get_fw_file_via_i(struct fts_8006ab_upgrade *upg)
{
...
/*****************************************************************************
 *  Name: fts_8006ab_fwupg_get_fw_file
 ...
 *****************************************************************************/
static int fts_8006ab_fwupg_get_fw_file(struct fts_8006ab_upgrade *upg)
{
    int ret = 0;
    ...
    ret = fts_8006ab_fwupg_get_module_info(upg);
    ...
    if (FTS_FW_REQUEST_SUPPORT) {
        msleep(500);
        ret = fts_8006ab_get_fw_file_via_request_firmware(upg);
        if (ret != 0) {
            get_fw_i_flag = true;
        }
    } else {
    ...
    if (get_fw_i_flag) {
        ret = fts_8006ab_get_fw_file_via_i(upg);
    }
...
static void fts_8006ab_fwupg_work(struct work_struct *work)
{
    int ret = 0;
    u8 chip_id = 0;
    struct fts_8006ab_upgrade *upg = fwupgrade8006ab;
    ...
    /* get fw */
    ret = fts_8006ab_fwupg_get_fw_file(upg);
    if (ret < 0) {
    ...
    ret = fts_8006ab_fw_download(upg->fw, upg->fw_length, true);
    if (ret < 0) {
        ...
        ret = fts_8006ab_read_reg(FTS_REG_CHIP_ID, &chip_id);
        FTS_INFO("read chip id:0x%02x", chip_id);
         /* prize modify by yangcong for  20201125 start */
        #ifdef CONFIG_PRIZE_HARDWARE_INFO
        	fts_8006ab_read_reg(FTS_REG_FW_VER, &ctp_fw_version);
...
{
    struct upgrade_setting_nf *setting = &upgrade_8006ab__setting_list[0];
    int setting_count =
        sizeof(upgrade_8006ab__setting_list) / sizeof(upgrade_8006ab__setting_list[0]);
    ...
    fwupgrade8006ab = (struct fts_8006ab_upgrade *)kzalloc(sizeof(*fwupgrade8006ab), GFP_KERNEL);
    if (NULL == fwupgrade8006ab) {
    ...
    fwupgrade8006ab->setting_nf = setting;

    if (NULL == fwupgrade8006ab->setting_nf) {
        FTS_ERROR("no upgrade settings match, can't upgrade");
        kfree(fwupgrade8006ab);
        fwupgrade8006ab = NULL;
        return -ENODATA;
    }
    ...
    fwupgrade8006ab->ts_data = ts_data;
    INIT_WORK(&ts_data->fwupg_work, fts_8006ab_fwupg_work);
...
int fts_8006ab_fwupg_exit(struct fts_8006ab_ts_data *ts_data)
{
    FTS_FUNC_ENTER();
    if (fwupgrade8006ab) {
        if (fwupgrade8006ab->fw_from_request) {
            vfree(fwupgrade8006ab->fw);
            fwupgrade8006ab->fw = NULL;
        }
