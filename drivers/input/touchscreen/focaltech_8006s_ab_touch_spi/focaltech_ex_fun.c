...
static ssize_t fts_8006ab_debug_write(
    struct file *filp, const char __user *buff, size_t count, loff_t *ppos)
{
    ...
    case PROC_SET_TEST_FLAG:
        ...
        if (writebuf[1] == 0) {
#if FTS_ESDCHECK_EN
            fts_8006ab_esdcheck_switch(ENABLE);
#endif
        } else {
#if FTS_ESDCHECK_EN
            fts_8006ab_esdcheck_switch(DISABLE);
#endif
        }
        break;
    ...
    case PROC_WRITE_REGISTER:
        ret = fts_8006ab_write_reg(writebuf[1], writebuf[2]);
        if (ret < 0) {
    ...
    case PROC_WRITE_DATA:
        writelen = buflen - 1;
        ret = fts_8006ab_write(writebuf + 1, writelen);
        if (ret < 0) {
    ...
    case PROC_ENTER_TEST_ENVIRONMENT:
        ...
        if (0 == writebuf[1]) {
            fts_8006ab_enter_test_environment(0);
        } else {
            fts_8006ab_enter_test_environment(1);
        }
        break;
...
static ssize_t fts_8006ab_debug_read(
    struct file *filp, char __user *buff, size_t count, loff_t *ppos)
{
...
#if FTS_ESDCHECK_EN
    fts_8006ab_esdcheck_proc_busy(1);
#endif

    switch (proc->opmode) {
    case PROC_READ_REGISTER:
        num_read_chars = 1;
        ret = fts_8006ab_read_reg(proc->cmd[0], &readbuf[0]);
        if (ret < 0) {
    ...
    case PROC_READ_DATA:
        ...
        ret = fts_8006ab_read(proc->cmd, proc->cmd_len, readbuf, num_read_chars);
        if (ret < 0) {
...
#if FTS_ESDCHECK_EN
    fts_8006ab_esdcheck_proc_busy(0);
#endif
...
static const struct file_operations fts_8006ab_proc_fops = {
    .owner  = THIS_MODULE,
    .read   = fts_8006ab_debug_read,
    .write  = fts_8006ab_debug_write,
};
#else
static int fts_8006ab_debug_write(
...
static int fts_8006ab_debug_read(
...
int fts_8006ab_create_apk_debug_channel(struct fts_8006ab_ts_data *ts_data)
{
    struct ftxxxx_proc *proc = &ts_data->proc;
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 10, 0))
    proc->proc_entry = proc_create(PROC_NAME, 0777, NULL, &fts_8006ab_proc_fops);
    ...
    proc->proc_entry->write_proc = fts_8006ab_debug_write;
    proc->proc_entry->read_proc = fts_8006ab_debug_read;
#endif
...
void fts_8006ab_release_apk_debug_channel(struct fts_8006ab_ts_data *ts_data)
{
    struct ftxxxx_proc *proc = &ts_data->proc;
 ...
 * sysfs interface
 ***********************************************************************/
/* fts_8006ab_hw_reset interface */
static ssize_t fts_8006ab_hw_reset_show(
...
static ssize_t fts_8006ab_hw_reset_store(
    struct device *dev,
...
/* fts_8006ab_irq interface */
static ssize_t fts_8006ab_irq_show(
    struct device *dev, struct device_attribute *attr, char *buf)
{
    ssize_t count = 0;
    struct irq_desc *desc = irq_to_desc(fts_8006ab_data->irq);
...
static ssize_t fts_8006ab_irq_store(
    struct device *dev,
    struct device_attribute *attr, const char *buf, size_t count)
{
    struct input_dev *input_dev = fts_8006ab_data->input_dev;
    ...
    if (FTS_SYSFS_ECHO_ON(buf)) {
        FTS_INFO("enable irq");
        fts_8006ab_irq_enable();
    } else if (FTS_SYSFS_ECHO_OFF(buf)) {
        FTS_INFO("disable irq");
        fts_8006ab_irq_disable();
    }
    mutex_unlock(&input_dev->mutex);
    return count;
}

/* fts_8006ab_boot_mode interface */
static ssize_t fts_8006ab_bootmode_store(
    struct device *dev,
    struct device_attribute *attr, const char *buf, size_t count)
{
    struct input_dev *input_dev = fts_8006ab_data->input_dev;
    ...
    if (FTS_SYSFS_ECHO_ON(buf)) {
        ...
        fts_8006ab_data->fw_is_running = false;
    } else if (FTS_SYSFS_ECHO_OFF(buf)) {
        FTS_INFO("[EX-FUN]set to fw mode");
        fts_8006ab_data->fw_is_running = true;
    }
    mutex_unlock(&input_dev->mutex);
    FTS_FUNC_EXIT();
...
static ssize_t fts_8006ab_bootmode_show(
    struct device *dev, struct device_attribute *attr, char *buf)
{
    ssize_t count = 0;
    struct input_dev *input_dev = fts_8006ab_data->input_dev;
    ...
    FTS_FUNC_ENTER();
    mutex_lock(&input_dev->mutex);
    if (true == fts_8006ab_data->fw_is_running) {
...
/* fts_8006ab_tpfwver interface */
static ssize_t fts_8006ab_tpfwver_show(
    struct device *dev, struct device_attribute *attr, char *buf)
{
    int ret = 0;
    struct fts_8006ab_ts_data *ts_data = fts_8006ab_data;
    ...
    fts_8006ab_esdcheck_proc_busy(1);
#endif
    ret = fts_8006ab_read_reg(FTS_REG_FW_VER, &fwver);
#if FTS_ESDCHECK_EN
    fts_8006ab_esdcheck_proc_busy(0);
#endif
...
static ssize_t fts_8006ab_tpfwver_store(
    struct device *dev,
    struct device_attribute *attr, const char *buf, size_t count)
{
    return -EPERM;
...
/* fts_8006ab_rw_reg */
static ssize_t fts_8006ab_tprwreg_show(
    struct device *dev, struct device_attribute *attr, char *buf)
{
    int count;
    int i;
 ...
 * [5-6]...[n-(n+1)]: data, hex
 */
static int fts_8006ab_parse_buf(const char *buf, size_t cmd_len)
{
    int length;
    int i;
    char *tmpbuf;
...
static ssize_t fts_8006ab_tprwreg_store(
    struct device *dev,
    struct device_attribute *attr, const char *buf, size_t count)
{
    struct input_dev *input_dev = fts_8006ab_data->input_dev;
    ...
    if (2 == cmd_length) {
        ...
        rw_op.len = fts_8006ab_parse_buf(buf, cmd_length);
    }
    ...
    if (rw_op.len < 0) {
        ...
        if (RWREG_OP_READ == rw_op.type) {
            if (rw_op.len == 1) {
                u8 reg, val;
                reg = rw_op.reg & 0xFF;
                rw_op.res = fts_8006ab_read_reg(reg, &val);
                rw_op.val = val;
            } else {
                char reg;
                reg = rw_op.reg & 0xFF;

                rw_op.res = fts_8006ab_read(&reg, 1, rw_op.opbuf, rw_op.len);
            }
            ...
            if (rw_op.len == 1) {
                ...
                reg = rw_op.reg & 0xFF;
                val = rw_op.val & 0xFF;
                rw_op.res = fts_8006ab_write_reg(reg, val);
            } else {
                rw_op.res = fts_8006ab_write(rw_op.opbuf, rw_op.len);
            }
            if (rw_op.res < 0) {
...
/* fts_8006ab_upgrade_bin interface */
static ssize_t fts_8006ab_fwupgradebin_show(
    struct device *dev, struct device_attribute *attr, char *buf)
{
    return -EPERM;
}

static ssize_t fts_8006ab_fwupgradebin_store(
    struct device *dev,
    struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    mutex_lock(&input_dev->mutex);
    fts_8006ab_upgrade_bin(fwname, 0);
    mutex_unlock(&input_dev->mutex);

    return count;
}

/* fts_8006ab_force_upgrade interface */
static ssize_t fts_8006ab_fwforceupg_show(
    struct device *dev, struct device_attribute *attr, char *buf)
{
    return -EPERM;
}

static ssize_t fts_8006ab_fwforceupg_store(
    struct device *dev,
    struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    mutex_lock(&input_dev->mutex);
    fts_8006ab_upgrade_bin(fwname, 1);
    mutex_unlock(&input_dev->mutex);

    return count;
}

/* fts_8006ab_driver_info interface */
static ssize_t fts_8006ab_driverinfo_show(
    struct device *dev, struct device_attribute *attr, char *buf)
{
    int count = 0;
    struct fts_8006ab_ts_data *ts_data = fts_8006ab_data;
...
static ssize_t fts_8006ab_driverinfo_store(
    struct device *dev,
    struct device_attribute *attr, const char *buf, size_t count)
{
    return -EPERM;
...
/* fts_8006ab_dump_reg interface */
static ssize_t fts_8006ab_dumpreg_show(
    struct device *dev, struct device_attribute *attr, char *buf)
{
    int count = 0;
    u8 val = 0;
...
static ssize_t fts_8006ab_dumpreg_store(
    struct device *dev,
    struct device_attribute *attr, const char *buf, size_t count)
{
    return -EPERM;
...
/* fts_8006ab_dump_reg interface */
static ssize_t fts_8006ab_tpbuf_show(
    struct device *dev, struct device_attribute *attr, char *buf)
{
    int count = 0;
    int i = 0;
    ...
    for (i = 0; i < fts_8006ab_data->pnt_buf_size; i++) {
        count += snprintf(buf + count, PAGE_SIZE, "%02x ", fts_8006ab_data->point_buf[i]);
    }
    count += snprintf(buf + count, PAGE_SIZE, "\n");
    mutex_unlock(&input_dev->mutex);
...
static ssize_t fts_8006ab_tpbuf_store(
    struct device *dev,
    struct device_attribute *attr, const char *buf, size_t count)
{
    return -EPERM;
...
/* fts_8006ab_log_level interface */
static ssize_t fts_8006ab_log_level_show(
    struct device *dev, struct device_attribute *attr, char *buf)
{
    int count = 0;
    struct input_dev *input_dev = fts_8006ab_data->input_dev;
    ...
    count += snprintf(buf + count, PAGE_SIZE, "log level:%d\n",
                      fts_8006ab_data->log_level);
    mutex_unlock(&input_dev->mutex);

    return count;
}

static ssize_t fts_8006ab_log_level_store(
    struct device *dev,
    struct device_attribute *attr, const char *buf, size_t count)
{
    int value = 0;
    ...
    mutex_lock(&input_dev->mutex);
    sscanf(buf, "%d", &value);
    FTS_DEBUG("log level:%d->%d", fts_8006ab_data->log_level, value);
    fts_8006ab_data->log_level = value;
    mutex_unlock(&input_dev->mutex);
    FTS_FUNC_EXIT();
...
static DEVICE_ATTR(fts_8006ab_fw_version, S_IRUGO | S_IWUSR, fts_8006ab_tpfwver_show, fts_8006ab_tpfwver_store);
...
*       cat rw_reg
*/
static DEVICE_ATTR(fts_8006ab_rw_reg, S_IRUGO | S_IWUSR, fts_8006ab_tprwreg_show, fts_8006ab_tprwreg_store);
/*  upgrade from fw bin file   example:echo "*.bin" > fts_8006ab_upgrade_bin */
static DEVICE_ATTR(fts_8006ab_upgrade_bin, S_IRUGO | S_IWUSR, fts_8006ab_fwupgradebin_show, fts_8006ab_fwupgradebin_store);
static DEVICE_ATTR(fts_8006ab_force_upgrade, S_IRUGO | S_IWUSR, fts_8006ab_fwforceupg_show, fts_8006ab_fwforceupg_store);
static DEVICE_ATTR(fts_8006ab_driver_info, S_IRUGO | S_IWUSR, fts_8006ab_driverinfo_show, fts_8006ab_driverinfo_store);
static DEVICE_ATTR(fts_8006ab_dump_reg, S_IRUGO | S_IWUSR, fts_8006ab_dumpreg_show, fts_8006ab_dumpreg_store);
static DEVICE_ATTR(fts_8006ab_hw_reset, S_IRUGO | S_IWUSR, fts_8006ab_hw_reset_show, fts_8006ab_hw_reset_store);
static DEVICE_ATTR(fts_8006ab_irq, S_IRUGO | S_IWUSR, fts_8006ab_irq_show, fts_8006ab_irq_store);
static DEVICE_ATTR(fts_8006ab_boot_mode, S_IRUGO | S_IWUSR, fts_8006ab_bootmode_show, fts_8006ab_bootmode_store);
static DEVICE_ATTR(fts_8006ab_touch_point, S_IRUGO | S_IWUSR, fts_8006ab_tpbuf_show, fts_8006ab_tpbuf_store);
static DEVICE_ATTR(fts_8006ab_log_level, S_IRUGO | S_IWUSR, fts_8006ab_log_level_show, fts_8006ab_log_level_store);

/* add your attr in here*/
static struct attribute *fts_8006ab_attributes[] = {
    &dev_attr_fts_8006ab_fw_version.attr,
    &dev_attr_fts_8006ab_rw_reg.attr,
    &dev_attr_fts_8006ab_dump_reg.attr,
    &dev_attr_fts_8006ab_upgrade_bin.attr,
    &dev_attr_fts_8006ab_force_upgrade.attr,
    &dev_attr_fts_8006ab_driver_info.attr,
    &dev_attr_fts_8006ab_hw_reset.attr,
    &dev_attr_fts_8006ab_irq.attr,
    &dev_attr_fts_8006ab_boot_mode.attr,
    &dev_attr_fts_8006ab_touch_point.attr,
    &dev_attr_fts_8006ab_log_level.attr,
    NULL
};

static struct attribute_group fts_8006ab_attribute_group = {
    .attrs = fts_8006ab_attributes
};
...
int fts_8006ab_create_sysfs(struct fts_8006ab_ts_data *ts_data)
{
    ...
    ret = sysfs_create_group(&ts_data->dev->kobj, &fts_8006ab_attribute_group);
    if (ret) {
        FTS_ERROR("[EX]: sysfs_create_group() failed!!");
        sysfs_remove_group(&ts_data->dev->kobj, &fts_8006ab_attribute_group);
        return -ENOMEM;
    } else {
