...
void tpd_get_dts_info(void)
{
	...
	if (node1) {
		...
		pr_err("can't find touch compatible custom node\n");
	}
}
...
static int tpd_suspend(struct platform_device *pdev, pm_message_t state)
{
	//pr_err("gezi------%s-------------%d\n",__func__,__LINE__);
	return 0;
}

static int tpd_resume(struct platform_device *pdev)
{
	//pr_err("gezi------%s-------------%d\n",__func__,__LINE__);
	return 0;
}
...
//add by prize modified tp resolution 20211014 --start
#if 0
	if (2560 == TPD_RES_X)
		TPD_RES_X = 2048;
	if (1600 == TPD_RES_Y)
		TPD_RES_Y = 1536;
#endif
//add by prize modified tp resolution 20211014 --end
