...
#define FW_BIN_NAME "GC7202_panda6.517_K6520_0x0307.0x0B0C_20220309.bin"
...
/* gcore_reset();  //reset RESX pin */
	...
	if (!wait_for_completion_interruptible_timeout(&fw_update_complete, HZ/2)) {
...
void gcore_request_firmware_update_work(struct work_struct *work)
{
	...
	mod_delayed_work(gdev_fwu->fwu_workqueue, &gdev_fwu->wdt_work, 
					   msecs_to_jiffies(GCORE_WDT_TIMEOUT_PERIOD + 1000));
#endif

	g_ret_update = 1;
	retry_count = 0;
	gdev_fwu->ts_stat = TS_NORMAL;	
...
fail1:
	...
	if( retry_count < 3 ){
		...
		GTP_ERROR("trigger display esd recovery");
		retry_count = 0;
		primary_display_esd_recovery();
		//gc2702_hdplus_dsi_vdo_incell_lcm_drv.suspend();
		//msleep(1);
		//gc2702_hdplus_dsi_vdo_incell_lcm_drv.resume();
	}
