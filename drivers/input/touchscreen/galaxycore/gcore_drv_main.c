...
static int gcore_fb_notifier_callback(struct notifier_block *self, unsigned long event, void *data)
{
	...
	switch (blank) {
		case FB_BLANK_UNBLANK:
			//GTP_ERROR("gezi [%s] screen on,is_init = %d\n",__func__,is_init);
			//if(!is_init){
			//	return 0;
			//}
			gcore_resume();
		...
		case FB_BLANK_POWERDOWN:
			//GTP_ERROR("gezi [%s] screen off,is_init = %d\n",__func__,is_init);
			//is_init = 1;
			gcore_suspend();
			break;
		default:
...
static struct notifier_block gcore_fb_notifier = {
	.notifier_call = gcore_fb_notifier_callback,
};
...
/*	struct input_dev *input_dev = gcore_input_init();
	
	if(input_dev == NULL){
		return -1;
	}
	else{
		gdev->input_device = input_dev;
	}
*/
...
//	if (gdev->ts_stat) {
//		GTP_DEBUG("ts_stat not in normal,not handle");
//		return 0;
//	}
	GTP_DEBUG("event ts_stat %d", gdev->ts_stat);
...
int gcore_probe_device(void)
{
	...
	strcpy(current_tp_info.vendor, "Galaxycore");
...
//		gcore_idm_tddi_reset();

//		mod_delayed_work(gdev->fwu_workqueue, &gdev->wdt_work, 
//						   msecs_to_jiffies(GCORE_WDT_TIMEOUT_PERIOD + 2000));
...
void gcore_spi_config(void)
{
	...
	gcore_spi_slave->max_speed_hz = 8000000;//10000000   10M
...
static s32 gcore_spi_probe(struct spi_device *slave)
{
	...
	ret = fb_register_client(&gcore_fb_notifier);
	if (ret){
		GTP_ERROR("gezi [%s] failed to register gcore_fb_notifier %d\n", __func__, ret);
	}
