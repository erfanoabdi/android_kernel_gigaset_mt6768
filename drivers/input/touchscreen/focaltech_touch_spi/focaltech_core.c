...
static int fts_get_ic_information(struct fts_ts_data *ts_data)
{
    ...
    for (cnt = 0; cnt < 3; cnt++) {
        ...
        if (ret < 0) {
            FTS_DEBUG("can't get ic informaton,retry:%dchip_id[0]=%d,chip_id[1]=%d", cnt,chip_id[0],chip_id[1]);
            continue;
        }
...
/* prize add  for sleep current optimization start */
void fts_ts_early_suspend(void)
{
	int ret;
    struct fts_ts_data *ts_data = fts_data;
...
/* prize add  for sleep current optimization end */
#elif defined(CONFIG_HAS_EARLYSUSPEND)
...
static int fts_ts_probe_entry(struct fts_ts_data *ts_data)
{
 ...
 sprintf(current_tp_info.id,"0x%02x%02x", ts_data->ic_info.ids.chip_idh,ts_data->ic_info.ids.chip_idl);
	strcpy(current_tp_info.chip, "FT8006S_AA");
