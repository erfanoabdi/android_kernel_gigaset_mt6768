...
//if use compat, should define REE_SECSPI_SUPPORT and TEE_VENDOR_XXXX
...
#include "fp_func.h"
struct TEEC_UUID cdfinger_uuid = {0x7778c03f, 0xc30c, 0x4dd0,{0xa3, 0x19, 0xea, 0x29, 0x64, 0x3d, 0x4d, 0x4b}};
#endif
...
static int cdfinger_parse_dts(struct cdfinger_data *cdfinger)
{
	...
	struct device *dev = &cdfinger->spi->dev;
#endif
	dev->of_node = of_find_compatible_node(NULL,NULL,"mediatek,mt6765-fpc");
...
static void cdfinger_ws_timeout(struct cdfinger_data *cdfinger)
{
	...
	wake_lock_timeout(&cdfinger->cdfinger_lock,1*HZ);
#endif
}
...
#endif
{
	...
	CDFINGER_DBG("11111111111111111 exit\n");
