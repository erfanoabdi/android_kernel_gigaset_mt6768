...
#if 0//prize wyq 20191019 move hw init to lk
...
int  aw99703_set_brightness(struct aw99703_data *drvdata, int brt_val)
{
	...
	if (drvdata->enable == false) {
		//aw99703_backlight_init(drvdata);//prize wyq 
...
int chargepump_set_backlight_level(unsigned int level)
{
	...
	//printk("%s aw99703_brt_level is %d \n",__func__, level);

	if (g_aw99703_data->enable == false) {
		//aw99703_backlight_init(g_aw99703_data);//prize wyq
		aw99703_backlight_enable(g_aw99703_data);
	}

	level = aw99703_brightness_map(level);
//prize add by lipengpeng 20201105 start
if((level>0)&&(level <= 12))
	 level=12;
//prize add by lipengpeng 20201105 end 
	if (level > 0) {
		/*enalbe bl mode*/
		/* set backlight level */
...
aw99703_get_dt_data(struct device *dev, struct aw99703_data *drvdata)
{
	...
	drvdata->hwen_gpio = of_get_named_gpio(np, "hwen-gpio", 0);
	pr_info("%s drvdata->hwen_gpio --<%d>\n", __func__, drvdata->hwen_gpio);

	rc = of_property_read_u32(np, "pwm-mode", &drvdata->pwm_mode);
	if (rc != 0)
		pr_err("%s pwm-mode not found\n", __func__);
	else
		pr_info("%s pwm_mode=%d\n", __func__, drvdata->pwm_mode);

	drvdata->using_lsb = of_property_read_bool(np, "using-lsb");
	...
	rc = of_property_read_u32(np, "bl-fscal-led", &temp);
	if (rc) {
	...
	rc = of_property_read_u32(np, "turn-on-ramp", &temp);
	if (rc) {
	...
	rc = of_property_read_u32(np, "turn-off-ramp", &temp);
	if (rc) {
	...
	rc = of_property_read_u32(np, "pwm-trans-dim", &temp);
	if (rc) {
	...
	rc = of_property_read_u32(np, "i2c-trans-dim", &temp);
	if (rc) {
	...
	rc = of_property_read_u32(np, "bl-channel", &bl_channel);
	if (rc) {
	...
	rc = of_property_read_u32(np, "bl-map", &drvdata->bl_map);
	if (rc != 0)
			...
			const struct i2c_device_id *id)
{
	...
	//prize wyq 20191018 remain lk initialization to keep brightness stable-start
	//aw99703_backlight_init(drvdata);
	//aw99703_backlight_enable(drvdata);

	//aw99703_set_brightness(drvdata, MAX_BRIGHTNESS);
	//prize wyq 20191018 remain lk initialization to keep brightness stable-end
