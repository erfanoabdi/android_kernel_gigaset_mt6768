...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	if(curr_sensor_id != 0)
	{
		*sensor_id = 0xFFFFFFFF;
        ...
        printk("[ov16A10gms_camera_sensor ] imgsensor_info.sensor_id:%x\n",imgsensor_info.sensor_id);
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		spin_lock(&imgsensor_drv_lock);
		imgsensor.i2c_write_id = imgsensor_info.i2c_addr_table[i];
                printk("[ov16A10gms_camera_sensor ] imgsensor_info.i2c_addr_table[i]:%x\n",imgsensor_info.i2c_addr_table[i]);
		spin_unlock(&imgsensor_drv_lock);
		do {	
			*sensor_id = (read_cmos_sensor_8(0x300A) << 16) | (read_cmos_sensor_8(0x300B)<<8) | read_cmos_sensor_8(0x300C);
                        ...
                        printk("[ov16A10gms_camera_sensor ] otp val:0x%x,0x%x,0x%x",gms_otp_value,gms_otp_value2,gms_otp_value3);
                        if(gms_otp_value == 0x55 && gms_otp_value2 == 0x55 && gms_otp_value3 == 0x55 )
                        {
                                *sensor_id = *sensor_id+1;
                        }

                        printk("[ov16A10gms_camera_sensor ] get_imgsensor_id :%x\n",*sensor_id);
