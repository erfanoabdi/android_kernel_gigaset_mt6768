...
int write_dualcam_cali_flag_file_s5kjn1(void)
{
...
int write_dualcam_cali_result_flag_file_s5kjn1(int result)
{
...
#if 0
int dump_dualcam_cali_flag_file_s5kjn1(void)
{
    struct file *eeprom_file = NULL;
    int err = 0;
    mm_segment_t fs = get_fs();
...
int dump_dualcam_cali_result_flag_file_s5kjn1(int result)
{
    struct file *result_file = NULL;
    int err = 0;
    mm_segment_t fs = get_fs();
...
int store_dualcam_cali_data_s5kjn1(void)
{
    int size = -1, rc = -1;
    BYTE checkSum = 0;
	int ret = 1;
    ...
    rc = write_dualcam_cali_flag_file_s5kjn1();
    if (rc < 0) {
    ...
    rc = write_dualcam_cali_result_flag_file_s5kjn1(ret);
    if (rc < 0) {
...
int dump_dualcam_cali_data_s5kjn1(void)
{
    struct file *eeprom_file = NULL;
...
#if 0
    err = dump_dualcam_cali_flag_file_s5kjn1();
    ...
    err = dump_dualcam_cali_result_flag_file_s5kjn1(ret2);
    if (err < 0) {
        printk("%s: failed to write_dualcam_cali_flag_file\n", __func__);
        //return -1;
        ret2 = -1;
