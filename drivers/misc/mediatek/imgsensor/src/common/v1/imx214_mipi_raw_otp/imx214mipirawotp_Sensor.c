...
#include "imx214mipirawotp_Sensor.h"

#define PFX "IMX214otp_camera_sensor"
...
static imgsensor_info_struct imgsensor_info = {
	.sensor_id = IMX214_SENSOR_OTP_ID,
...
static void hs_video_setting(void)
{
	...
	write_cmos_sensor(0x0221,0x11);
	write_cmos_sensor(0x0222,0x01);
	write_cmos_sensor(0x0340,0x05);// 05
	write_cmos_sensor(0x0341,0x00);//08
	write_cmos_sensor(0x0342,0x13);
	write_cmos_sensor(0x0343,0x90);
	write_cmos_sensor(0x0344,0x00);
	...
	write_cmos_sensor(0x0305,0x03);
	write_cmos_sensor(0x0306,0x00);
	write_cmos_sensor(0x0307,0x78);//79
	write_cmos_sensor(0x0309,0x0A);
	write_cmos_sensor(0x030B,0x01);
	write_cmos_sensor(0x0310,0x00);

	write_cmos_sensor(0x0820,0x0F);//0F
	write_cmos_sensor(0x0821,0x00);// 20
	write_cmos_sensor(0x0822,0x00);
	write_cmos_sensor(0x0823,0x00);
	write_cmos_sensor(0x3A03,0x06);
	write_cmos_sensor(0x3A04,0x68);
...
*
*************************************************************************/
static kal_uint16 GMS_OTP_VALUE = 0x0a;
static kal_uint16 read_otp_for_gms(void)
{
	write_cmos_sensor(0x0A02, 0x00); //Select Page
	write_cmos_sensor(0x0A00, 0x01); //Read ON
	return read_cmos_sensor(0x0A42);
}
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	kal_uint8 i = 0;
	kal_uint8 retry = 2;
	kal_uint16 gms_otp_value;
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		spin_unlock(&imgsensor_drv_lock);
		do {
			*sensor_id = ((read_cmos_sensor(0x0016) << 8) | read_cmos_sensor(0x0017)) + 2;
			gms_otp_value = read_otp_for_gms();
			if ( (*sensor_id == imgsensor_info.sensor_id) && (GMS_OTP_VALUE == gms_otp_value) ) {
				LOG_INF("i2c write id: 0x%x, sensor id: 0x%x, gms_otp_value: 0x%x\n", imgsensor.i2c_write_id,*sensor_id,gms_otp_value);
				/*iReadData(0x00,452,OTPData);*//*LukeHu--150810*/
				return ERROR_NONE;
			}
			LOG_INF("Read sensor id fail, write id:0x%x id: 0x%x, gms_otp_value: 0x%x\n", imgsensor.i2c_write_id,*sensor_id,gms_otp_value);
			retry--;
		} while(retry > 0);
		i++;
		retry = 2;
...
static kal_uint32 open(void)
{
	...
	if ( (imgsensor_info.sensor_id != sensor_id) || (GMS_OTP_VALUE != gms_otp_value) )
		return ERROR_SENSOR_CONNECT_FAIL;
