...
#include "ov5670gmsmipi_Sensor.h"

#ifdef CONFIG_COMPAT
#include <linux/compat.h>
#endif
#define PFX "ov5670gms_camera_sensor"
...
static imgsensor_info_struct imgsensor_info = {
	.sensor_id = OV5670GMSMIPI_SENSOR_ID,		//record sensor id defined in Kd_imgsensor.h
...
{
	LOG_INF("OV5670GMS_Sensor_Init_2lane E\n");

	write_cmos_sensor(0x0103,0x01);// ; software reset
	// mDELAY(10);
	write_cmos_sensor(0x303f, 0x01);//01);
	write_cmos_sensor(0x3004, 0x6c);
	write_cmos_sensor(0x3006, 0x6c);
	write_cmos_sensor(0x3012, 0x6c);//6c);
...
{
	LOG_INF(" OV5670GMSPreviewSetting_2lane enter\n");
...
{
	LOG_INF("OV5670GMSCaptureSetting_2lane enter! currefps:%d\n",currefps);
...
*
*************************************************************************/
//static kal_uint16 GMS_OTP_VALUE = 0x5;
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
			gms_otp_value = read_eeprom_module_id(0x0001);
			if(gms_otp_value == 0x5)
			{
...
UINT32 OV5670GMS_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
	...
	if (pfFunc!=NULL)
		*pfFunc=&sensor_func;
	return ERROR_NONE;
}	/*	OV5670GMSSUB_MIPI_RAW_SensorInit	*/
