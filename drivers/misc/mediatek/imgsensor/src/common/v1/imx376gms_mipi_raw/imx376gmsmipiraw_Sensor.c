	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id =( ((read_cmos_sensor_8(0x0016) << 8) |
			...
			printk("lens_id gms i2c write id: 0x%x, sensor id: 0x%x imgsensor_info.sensor_id: 0x%x\n",
			...
			/*zhaozhengtao 2016/02/19,modify for different module*/
				...
				if(lens_id == 0x11)
				{
					printk("gms lens_id success(0x%x) module_id(0x%x)",lens_id,imgsensor_info.module_id);
					return ERROR_NONE;
				}
				else
				{
					printk("gms lens_id fail(0x%x) module_id(0x%x)",lens_id,imgsensor_info.module_id);
