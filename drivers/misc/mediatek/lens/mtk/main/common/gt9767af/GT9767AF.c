...
static inline int moveAF(unsigned long a_u4Position)
{
	...
	while(abs(g_u4CurrPosition-g_u4TargetPosition) > 100)
	{
		if(g_u4CurrPosition<g_u4TargetPosition)
			g_u4CurrPosition += 100;
		else if(g_u4CurrPosition>g_u4TargetPosition)
			g_u4CurrPosition -= 100;
		s4AF_WriteReg(g_u4CurrPosition);
		mdelay(5);
//		LOG_INF("prize linchong move [curr] %d [target] %d\n", g_u4CurrPosition, g_u4TargetPosition); 
	}
