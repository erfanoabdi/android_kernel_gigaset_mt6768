...
static int i2c_read(u8 a_u2Addr, u8 *a_puBuff)
{
	...
	if (i4RetValue != 2) {
		printk("[DW9761AF_DRV]i2c_read: I2C write failed!!\n");
		return -1;
	}

	i4RetValue = i2c_master_recv(g_pstAF_I2Cclient, (char *)a_puBuff, 1);
	if (i4RetValue != 1) {
		printk("[DW9761AF_DRV]i2c_read: I2C read failed!!\n");
		return -1;
	}
...
static int s4DW9761AF_ReadReg(unsigned short *a_pu2Result)
{
...
static int s4AF_WriteReg(u16 a_u2Data)
{
	...
	if (i4RetValue < 0) {
		printk("[DW9761AF_DRV]i2c_read:I2C send failed!!\n");
		return -1;
	}
...
static void initdrv(void)
{
	...
	//pangfei modify for AF noise CR01651804 start
#ifdef ORIGINAL
	char puSendCmd3[2] = { 0x05, 0x65 };
#else
	char puSendCmd3[2] = { 0x05, 0x79 };
#endif
	//pangfei modify for AF noise CR01651804 end
...
static inline int moveAF(unsigned long a_u4Position)
{
	...
	if (*g_pAF_Opened == 1) {
		...
		initdrv();
		ret = s4DW9761AF_ReadReg(&InitPos);
