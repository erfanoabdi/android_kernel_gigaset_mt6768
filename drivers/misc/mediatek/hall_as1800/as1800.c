...
/*modified by custom, 20200921----start*/
static int hall_number = 0;//jason modify
/*modified by custom, 20200921----end*/
...
/*modified by custom, 20200921----start*/
static ssize_t notify_sendKeyEvent(int event, int number)//jason modify
{
    hall_key_event = event;
    hall_number = number;//jason modify
...
static void work_callback_hall_1(struct work_struct *work)
{
	if(g_cur_state_hall_1 ==  HALL_NEAR) 
	{
		...
		if( __gpio_get_value(eint_hall_1) == 0)
		{
			...
			notify_sendKeyEvent(HALL_NEAR, 1);//jason modify
		}
	}	
	else
	{
		...
		if( __gpio_get_value(eint_hall_1) == 1)
		{
			...
			notify_sendKeyEvent(HALL_FAR, 1);//jason modify
		}
	}	
...
static void work_callback_hall_2(struct work_struct *work)
{
	if(g_cur_state_hall_2 ==  HALL_NEAR) 
	{
		...
		if( __gpio_get_value(eint_hall_2) == 0)
		{
			...
			notify_sendKeyEvent(HALL_NEAR, 2);//jason modify
		}
	}	
	else
	{
		...
		if( __gpio_get_value(eint_hall_2) == 1)
		{
			...
			notify_sendKeyEvent(HALL_FAR, 2);//jason modify
		}
	}	
...
static irqreturn_t eint_func_hall_2(int irq,void *data)
{
	...
	HALL_DEBUG("[eint_func_hall_2] enter,g_cur_state_hall_2=%d,eint_type_hall_2=%d\n",g_cur_state_hall_2,eint_type_hall_2);
...
/*modified by custom, 20200921----start*/
		...
		/*prize-lixuefeng-20150602-start*/
            if(hall_number == 1){//jason modify
				input_report_key(hall_input_dev, KEY_F6, 1);
				input_report_key(hall_input_dev, KEY_F6, 0);
            }else {
		...
		/*prize-lixuefeng-20150602-start*/
            if(hall_number == 1){//jason modify
            	input_report_key(hall_input_dev, KEY_F5, 1);
            	input_report_key(hall_input_dev, KEY_F5, 0);
            }else {
...
/*modified by custom, 20200921----start*/
    __set_bit(KEY_F6, hall_input_dev->keybit);//jason add
    __set_bit(KEY_F5, hall_input_dev->keybit);//jason add
/*modified by custom, 20200921----end*/
...
/*modified by custom, 20200921----start*/	
//	hall_setup_eint(dev);//jason delete
