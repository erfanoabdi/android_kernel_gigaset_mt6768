...
static int gc0310_set_power(struct i2c_client *client,unsigned int enable)
{
    ...
    int  ret = 0;
	if (enable){
		if (gpio_is_valid(spc_data->vcama_pin)){
			gpio_request(spc_data->vcama_pin,client->name);
			gpio_direction_output(spc_data->vcama_pin,1);
			gpio_free(spc_data->vcama_pin);
			mdelay(2);
		}else if (spc_data->vcama_reg){
			ret = regulator_enable(spc_data->vcama_reg);
			if (ret < 0)
				CAMERA_DBG("enable regulator vcama_reg fail, ret=%d\n",ret);
		}

		if (gpio_is_valid(spc_data->vcamd_pin)){
			gpio_request(spc_data->vcamd_pin,client->name);
			gpio_direction_output(spc_data->vcamd_pin,1);
			gpio_free(spc_data->vcamd_pin);
			mdelay(2);
		}else if (spc_data->vcamd_reg){
			ret = regulator_enable(spc_data->vcamd_reg);
			if (ret < 0)
				CAMERA_DBG("enable regulator vcamd_reg fail, ret=%d\n",ret);
		}
		...
		if (gpio_is_valid(spc_data->vcamd_pin)){
			gpio_request(spc_data->vcamd_pin,client->name);
			gpio_direction_output(spc_data->vcamd_pin,0);
			gpio_free(spc_data->vcamd_pin);
		}else if (spc_data->vcamd_reg){
			ret = regulator_disable(spc_data->vcamd_reg);
		}

		if (gpio_is_valid(spc_data->vcama_pin)){
			gpio_request(spc_data->vcama_pin,client->name);
			gpio_direction_output(spc_data->vcama_pin,0);
			gpio_free(spc_data->vcama_pin);
		}else if (spc_data->vcama_reg){
			ret = regulator_disable(spc_data->vcama_reg);
		}
	}
    return ret;
}
...
static int gc0310_open(struct i2c_client *client)
{
	...
	CAMERA_DBG("%s Sensor Read ID OK \n",__func__);
