...
static void aw881xx_monitor_check_sysint(struct aw881xx *aw881xx)
{
	int ret = -1;
	uint16_t sysint;
	...
	if (sysint & (~AW881XX_BIT_SYSINT_CHECK_MASK)) {
		aw881xx->sysint_st = sysint;
		aw_dev_err(aw881xx->dev, "%s:check sysint fail, reg=0x%04x",
						__func__, sysint);
	}
}
