...
#define AW881XX_DRIVER_VERSION "v0.2.0"
...
static const char *const aw881xx_disp_on[] = {"on", "off"};	//prize add by huarui
...
//prize add  by lipengpeng 20210106 start 
static const struct snd_soc_dapm_widget aw881xx_dapm_widgets[]={
/* playback */
SND_SOC_DAPM_AIF_IN("AIF_RX","Speaker_Playback_6_36",0,SND_SOC_NOPM,0,0),
...
{
	if ((index >= aw881xx->prof_info.count)||(index < 0))//prize CVE-2017-7369
		return -EINVAL;
	else
		return 0;
...
static void aw881xx_start(struct aw881xx *aw881xx)
{
	...
	if (!re_inited){
		aw881xx_get_cali_re(&aw881xx->cali_attr); //prize huarui
	}
	//prize add by huarui end
...
//prize add by huarui, disable dsp start
static int aw881xx_dsp_on_info(struct snd_kcontrol *kcontrol,
			 struct snd_ctl_elem_info *uinfo)
{
	int count;
	...
	uinfo->type = SNDRV_CTL_ELEM_TYPE_ENUMERATED;
	uinfo->count = 1;
	count = ARRAY_SIZE(aw881xx_disp_on);
	...
	strlcpy(uinfo->value.enumerated.name,
		aw881xx_disp_on[uinfo->value.enumerated.item],
		sizeof(uinfo->value.enumerated.name));

	return 0;
}
static int aw881xx_dsp_on_get(struct snd_kcontrol *kcontrol,
			struct snd_ctl_elem_value *ucontrol)
{
	uint16_t reg = 0;
	aw_snd_soc_codec_t *codec =
	...
	aw881xx_i2c_read(aw881xx, AW881XX_REG_SYSCTRL, &reg);
	if (reg & 0x04){
...
static int aw881xx_dsp_on_set(struct snd_kcontrol *kcontrol,
			struct snd_ctl_elem_value *ucontrol)
{
	aw_snd_soc_codec_t *codec =
		aw_componet_codec_ops.aw_snd_soc_kcontrol_codec(kcontrol);
...
//prize add by huarui, disable dsp end
...
static int aw881xx_dynamic_create_common_controls(struct aw881xx *aw881xx)
{
	...
	//prize add by huarui, dsp enable , start
	aw881xx_dev_control[2].name = "aw881xx_dsp";
	aw881xx_dev_control[2].iface = SNDRV_CTL_ELEM_IFACE_MIXER;
	aw881xx_dev_control[2].info = aw881xx_dsp_on_info;
	aw881xx_dev_control[2].get = aw881xx_dsp_on_get;
	aw881xx_dev_control[2].put = aw881xx_dsp_on_set;
	//prize add by huarui, dsp enable , end
...
//prize add by lipengpeng 20210330 startr 
    aw881xx->status = AW881XX_PW_ON;
	aw881xx_stop(aw881xx);
...
static struct snd_soc_dapm_widget aw881xx_dapm_widgets_common[] = {
	/* Stream widgets */
	SND_SOC_DAPM_AIF_IN("AIF IN", "Speaker_Playback",
		 0, SND_SOC_NOPM, 0, 0),
	SND_SOC_DAPM_AIF_OUT("AIF OUT", "Speaker_Capture",
		0, SND_SOC_NOPM, 0, 0),
...
static const struct snd_soc_dapm_route aw881xx_dapm_routes_common[] = {
	{ "OUTL", NULL, "AIF IN" },
	{ "AIF OUT", NULL, "AEC Loopback" },
};

static void aw881xx_add_widgets(struct aw881xx *aw881xx)
{
	struct snd_soc_dapm_context *dapm;
	struct snd_soc_dapm_widget *widgets;
	//unsigned int num_dapm_widgets =
	//	ARRAY_SIZE(aw881xx_dapm_widgets_common);
//prize add by  lipengpeng 20220427 start 
	//dapm = snd_soc_codec_get_dapm(aw881xx->codec);
	dapm = snd_soc_component_get_dapm(aw881xx->codec);
//prize add by  lipengpeng 20220427 end 
	widgets = devm_kzalloc(&aw881xx->i2c->dev,
	...
	memcpy(widgets, aw881xx_dapm_widgets_common,
		sizeof(struct snd_soc_dapm_widget) *
		ARRAY_SIZE(aw881xx_dapm_widgets_common));

	snd_soc_dapm_new_controls(dapm, widgets,
			ARRAY_SIZE(aw881xx_dapm_widgets_common));
	snd_soc_dapm_add_routes(dapm, aw881xx_dapm_routes_common,
			ARRAY_SIZE(aw881xx_dapm_routes_common));
}
...
static int aw881xx_codec_probe(aw_snd_soc_codec_t *codec)
{
	...
	aw881xx_add_widgets(aw881xx);//prize huarui
				...
				size_t count)
{
	...
	if (val) {
		if (delayed_work_pending(&aw881xx->start_work))
			cancel_delayed_work_sync(&aw881xx->start_work);
